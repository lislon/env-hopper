...
import { useAuth, useAuthActions, useIsAuthenticated, useUser } from '~/modules/auth'
      ...
      <div className="flex items-center gap-4">
        ...
        {!isCatalogMode && (
          <div className="hidden md:flex items-center gap-1">
            ...
            {isAuthenticated && (
              <HeaderNavLink to="/admin" label="Admin" />
            )}
          </div>
        )}
      </div>
        ...
        {isLoading ? (
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="sm" className="w-8 h-8 rounded-full p-0 hover:bg-accent hover:text-accent-foreground cursor-pointer">
                {user.image ? (
                  <img
                    src={user.image}
                    alt={user.name}
