...
import { generateChecksum, getImageDimensions, getImageFormat, isRasterImage, resizeImage } from './assetUtils'
...
export function registerAssetRestController(
  ...
  router.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const assetType = (req.body['assetType'] as AssetType) || 'icon'
  ...
  router.get(`${basePath}/:id`, async (req: Request, res: Response) => {
    try {
      ...
      const shouldResize = resizeEnabled && isRasterImage(asset.mimeType) && !!width && Number.isFinite(width) && width > 0

      if (shouldResize) {
        const fmt = getImageFormat(asset.mimeType) || 'jpeg'
        const buf = await resizeImage(Buffer.from(asset.content), width, undefined, fmt)
  ...
  router.get(`${basePath}/by-name/:name`, async (req: Request, res: Response) => {
    try {
      ...
      const shouldResize = resizeEnabled && isRaster && !!width && Number.isFinite(width) && width > 0
      ...
      if (shouldResize) {
        ...
        let buf: Buffer
        const pipeline = sharp(Buffer.from(asset.content)).resize({ width, fit: 'inside', withoutEnlargement: true })
        if (fmt === 'png') {
