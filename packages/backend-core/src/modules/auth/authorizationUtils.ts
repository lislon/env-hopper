 ...
 */
export function getUserGroups(user: UserWithGroups | null | undefined): string[] {
  if (!user) {
    return []
  }
 ...
 */
export function getAdminGroupsFromEnv(): string[] {
  const adminGroups = process.env.AUTH_ADMIN_GROUPS || 'env_hopper_ui_super_admins'
  return adminGroups.split(',').map(g => g.trim()).filter(Boolean)
}
...
export function requireGroups(
  ...
  if (!isMemberOfAnyGroup(user, groups)) {
    throw new Error(`Forbidden: Membership in one of these groups required: ${groups.join(', ')}`)
  }
}
