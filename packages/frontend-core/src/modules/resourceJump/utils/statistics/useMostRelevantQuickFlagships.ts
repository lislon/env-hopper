...
export function useMostRelevantQuickFlaships(): Array<SlugAndDisplayName> {
  ...
  return useMemo(() => {
    const topN = getTopN({
      historyFiltered: history.filter(item => item.type === 'switch-selector' && item.flagshipSlug !== undefined),
      groupByFn: item => item.envSlug!,
      slugFinder: slug => flagshipJumpResources.find(env => env.slug === slug),
      n: MAX_N,
    });

    for (let c = 0; topN.length < MAX_N && c < flagshipJumpResources.length; c++) {
